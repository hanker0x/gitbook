...
## To detect Coin98 Extension with Arbitrum
...
```javascript
window.ethereum.request({ method: 'has_wallet', params: ['arbitrum'] })
// Example
window.ethereum.request({ method: 'has_wallet', params: ['arbitrum'] }).then(() => {
  // Wallet Exists
}).catch(e => { 
  // Wallet not found
})
