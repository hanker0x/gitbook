...
## To detect Coin98 Extension with EthereumPoW
...
```javascript
window.ethereum.request({ method: 'has_wallet', params: ['etherPoW'] })
// Example
window.ethereum.request({ method: 'has_wallet', params: ['etherPoW'] }).then(() => {
  // Wallet Exists
}).catch(e => { 
  // Wallet not found
})
...
Result - true if the token was added, fasle otherwise
