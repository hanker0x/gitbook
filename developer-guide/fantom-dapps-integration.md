...
## To detect Coin98 Extension with Fantom
...
```javascript
window.ethereum.request({ method: 'has_wallet', params: ['fantom'] })
// Example
window.ethereum.request({ method: 'has_wallet', params: ['fantom'] }).then(() => {
  // Wallet Exists
}).catch(e => { 
  // Wallet not found
})
