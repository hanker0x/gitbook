...
## To detect Coin98 Extension with GateChain
...
```javascript
window.ethereum.request({ method: 'has_wallet', params: ['gate'] })
// Example
window.ethereum.request({ method: 'has_wallet', params: ['gate'] }).then(() => {
  // Wallet Exists
}).catch(e => { 
  // Wallet not found
})
