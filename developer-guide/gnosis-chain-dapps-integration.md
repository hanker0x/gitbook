...
## To detect Coin98 Extension with Gnosis Chain
...
```javascript
window.ethereum.request({ method: 'has_wallet', params: ['xDai'] })
// Example
window.ethereum.request({ method: 'has_wallet', params: ['xDai'] }).then(() => {
  // Wallet Exists
}).catch(e => { 
  // Wallet not found
})
