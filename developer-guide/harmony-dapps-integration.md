...
## To detect Coin98 Extension with Harmony
...
```javascript
window.ethereum.request({ method: 'has_wallet', params: ['harmony'] })
// Example
window.ethereum.request({ method: 'has_wallet', params: ['harmony'] }).then(() => {
  // Wallet Exists
}).catch(e => { 
  // Wallet not found
})
