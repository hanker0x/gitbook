...
## To detect Coin98 Extension with Kardia
...
```javascript
window.ethereum.request({ method: 'has_wallet', params: ['kardia'] })
// Example
window.ethereum.request({ method: 'has_wallet', params: ['kardia'] }).then(() => {
  // Wallet Exists
}).catch(e => { 
  // Wallet not found
})
