...
## To detect Coin98 Extension with Klaytn
...
```javascript
window.ethereum.request({ method: 'has_wallet', params: ['klaytn'] })
// Example
window.ethereum.request({ method: 'has_wallet', params: ['klaytn'] }).then(() => {
  // Wallet Exists
}).catch(e => { 
  // Wallet not found
})
