...
## To detect Coin98 Extension with Optimism
...
```javascript
window.ethereum.request({ method: 'has_wallet', params: ['optimism'] })
// Example
window.ethereum.request({ method: 'has_wallet', params: ['optimism'] }).then(() => {
  // Wallet Exists
}).catch(e => { 
  // Wallet not found
})
